...
//======================================================================================
#include "stdafx.h"
#include <Nemesis/Gfx/Gui_d3d9.h>
...
namespace nemesis { namespace graphics 
{
	struct Gui_d3d9_s
	{
		...
		Allocator_t				Alloc;
		Gfx_d3d9_t				Gfx;
		Array<Texture_d3d9_t>	Garbage;
	};

	static void Gui_d3d9_Purge( Gui_d3d9_t gui )
	{
		for ( int i = 0; i < gui->Garbage.Count; ++i )
			Gfx_d3d9_Texture_Release( gui->Gfx, gui->Garbage[i] );
		gui->Garbage.Reset();
	}

	static bool NE_CALLBK Gui_CreateTexture( Renderer_t renderer, const TextureInfo_s& info, Texture_t* texture )
	{
		return Gfx_d3d9_Texture_Create( ((Gui_d3d9_s*)renderer)->Gfx, ToGfx(info), (Texture_d3d9_t*)(texture) );
	}

	static bool NE_CALLBK Gui_GetTextureInfo( Renderer_t renderer, Texture_t texture, TextureInfo_s& info )
	{
		TextureDesc_s desc = {};
		bool ok = Gfx_d3d9_Texture_GetDesc( ((Gui_d3d9_s*)renderer)->Gfx, (Texture_d3d9_t)texture, desc );
		info = ToGui(desc);
		return ok;
	}
	...
	{
		LockDesc_s lock = {};
		bool ok = Gfx_d3d9_Texture_Lock( ((Gui_d3d9_s*)renderer)->Gfx, (Texture_d3d9_t)texture, ToGfx(mode), lock );
		info = ToGui(lock);
		return ok;
	}
	...
	{
		return Gfx_d3d9_Texture_Unlock( ((Gui_d3d9_s*)renderer)->Gfx, (Texture_d3d9_t)texture );
	}

	static void NE_CALLBK Gui_ReleaseTexture( Renderer_t renderer, Texture_t texture )
	{
		((Gui_d3d9_s*)renderer)->Garbage.Append( (Texture_d3d9_t)texture );
	}
...
namespace nemesis { namespace graphics 
{
	Gui_d3d9_t Gui_d3d9_Create( Allocator_t alloc, Gfx_d3d9_t gfx )
	{
		Gui_d3d9_t out = Mem_Calloc<Gui_d3d9_s>( alloc );
	...
	void Gui_d3d9_Release( Gui_d3d9_t gui )
	{
		if (!gui)
			return;
		Gui_d3d9_Purge( gui );
	...
	void Gui_d3d9_NextFrame( Gui_d3d9_t gui )
	{
		Gui_d3d9_Purge( gui );
	}
